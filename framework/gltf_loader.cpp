...
namespace clan
{
    namespace
    {
        ...
        inline void upload_image_to_gpu(CommandBuffer &command_buffer, Buffer &staging_buffer, sg::Image &image)
        {
    ...
    std::unique_ptr<sg::Scene> GLTFLoader::read_scene_from_file(const std::string &file_name, int scene_index)
    {
        ...
        std::string gltf_file = fs::path::get(fs::path::Type::Assets) + file_name;
    ...
    sg::Scene GLTFLoader::load_scene(int scene_index)
    {
        ...
        LOGI("Time spent loading images: {} seconds across {} threads.", to_string(elapsed_time), thread_count);
        ...
        for (auto &gltf_material : model.materials)
        {
            ...
            for (auto &gltf_value : gltf_material.values)
            {
                if (gltf_value.first.find("Texture") != std::string::npos)
                {
                    ...
                    sg::Texture *tex = textures.at(gltf_value.second.TextureIndex());
        ...
        for (auto &gltf_mesh : model.meshes)
        {
            ...
            for (size_t i_primitive = 0; i_primitive < gltf_mesh.primitives.size(); i_primitive++)
            {
                ...
                if (gltf_primitive.indices >= 0)
                {
                    ...
                    submesh->index_buffer = std::make_unique<Buffer>(device,
                                                                        index_data.size(),
        ...
        if (!scene.has_component<sg::Light>())
        {
            ...
            add_directional_light(scene, glm::quat({glm::radians(-90.0f), 0.0f, glm::radians(30.0f)}));
        }
    ...
    std::unique_ptr<sg::Image> GLTFLoader::parse_image(tinygltf::Image &gltf_image) const
    {
        ...
        else
        {
            ...
            LOGW("the image file uri is {}", image_uri);
            image          = sg::Image::load(gltf_image.name, image_uri, sg::Image::Unknown);
        }
