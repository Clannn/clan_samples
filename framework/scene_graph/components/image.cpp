...
namespace clan
{
    namespace sg
    {
        ...
        void Image::create_vk_image(Device const &device, VkImageViewType image_view_type, VkImageCreateFlags flags)
        {
            ...
            vk_image = std::make_unique<clan::Image>(device,
                                                     get_extent(),
                                                     format,
            ...
            vk_image_view = std::make_unique<clan::ImageView>(*vk_image, image_view_type);
            vk_image_view->set_debug_name("View on " + get_name());
        }

        const clan::Image &Image::get_vk_image() const
        {
            assert(vk_image && "Vulkan image was not created");
            return *vk_image;
        }

        const clan::ImageView &Image::get_vk_image_view() const
        {
