...
namespace clan
{
     ...
     * The lifecycle of a Clan sample starts by instantiating the correct Platform
     ...
     * to then call ClanSample::prepare() to prepare the Clan sample when it is ready.
     *
     * @subsection sample_init Sample initialization
     * The preparation step is divided in two steps, one in ClanSample and the other in the
     * specific sample, such as SurfaceRotation.
     * ClanSample::prepare() contains functions that do not require customization,
     ...
     * Typically a sample will then call ClanSample::update().
     *
     ...
     * - calling ClanSample::draw_swapchain_renderpass (see below)
     ...
     * - ResourceCache
     * - BufferPool
     * - Core classes: Classes in clan wrap Vulkan objects for indexing and hashing.
     */
    class ClanSample : public Application
    {
    public:
        ClanSample() = default;
    ...
    protected:
         ...
         */
        static void set_viewport_and_scissor(CommandBuffer &command_buffer, const VkExtent2D &extent);
