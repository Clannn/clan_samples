...
namespace clan
{
        ...
        stats_view(stats)
    {
        ...
        font_image = std::make_unique<Image>(device, font_extent, VK_FORMAT_R8G8B8A8_UNORM,
        ...
        clan::ShaderSource vert_shader("imgui.vert");
        clan::ShaderSource frag_shader("imgui.frag");

        std::vector<clan::ShaderModule *> shader_modules;
        ...
        sampler = std::make_unique<Sampler>(device, sampler_info);
        sampler->set_debug_name("GUI sampler");

        if (explicit_update)
        {
            vertex_buffer = std::make_unique<Buffer>(sample.get_render_context().get_device(), 1, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_TO_CPU);
            vertex_buffer->set_debug_name("GUI vertex buffer");

            index_buffer = std::make_unique<Buffer>(sample.get_render_context().get_device(), 1, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VMA_MEMORY_USAGE_GPU_TO_CPU);
    ...
    void Gui::prepare(const VkPipelineCache pipeline_cache, const VkRenderPass render_pass, const std::vector<VkPipelineShaderStageCreateInfo> &shader_stages)
    {
        ...
        std::vector<VkDescriptorPoolSize> pool_sizes = {
            clan::initializers::descriptor_pool_size(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1)};
        VkDescriptorPoolCreateInfo descriptorPoolInfo = clan::initializers::descriptor_pool_create_info(pool_sizes, 2);
        ...
        std::vector<VkDescriptorSetLayoutBinding> layout_bindings = {
            clan::initializers::descriptor_set_layout_binding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 0),
        };
        VkDescriptorSetLayoutCreateInfo descriptor_set_layout_create_info = clan::initializers::descriptor_set_layout_create_info(layout_bindings);
        ...
        // Descriptor set
        VkDescriptorSetAllocateInfo descriptor_allocation = clan::initializers::descriptor_set_allocate_info(descriptor_pool, &descriptor_set_layout, 1);
        VK_CHECK(vkAllocateDescriptorSets(sample.get_render_context().get_device().get_handle(), &descriptor_allocation, &descriptor_set));
        VkDescriptorImageInfo font_descriptor = clan::initializers::descriptor_image_info(
            sampler->get_handle(),
            font_image_view->get_handle(),
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
        std::vector<VkWriteDescriptorSet> write_descriptor_sets = {
            clan::initializers::write_descriptor_set(descriptor_set, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, &font_descriptor)};
        ...
        VkPipelineInputAssemblyStateCreateInfo input_assembly_state =
            clan::initializers::pipeline_input_assembly_state_create_info(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0, VK_FALSE);

        VkPipelineRasterizationStateCreateInfo rasterization_state =
            clan::initializers::pipeline_rasterization_state_create_info(VK_POLYGON_MODE_FILL, VK_CULL_MODE_NONE, VK_FRONT_FACE_COUNTER_CLOCKWISE);
        ...
        VkPipelineColorBlendStateCreateInfo color_blend_state =
            clan::initializers::pipeline_color_blend_state_create_info(1, &blend_attachment_state);

        VkPipelineDepthStencilStateCreateInfo depth_stencil_state =
            clan::initializers::pipeline_depth_stencil_state_create_info(VK_FALSE, VK_FALSE, VK_COMPARE_OP_ALWAYS);

        VkPipelineViewportStateCreateInfo viewport_state =
            clan::initializers::pipeline_viewport_state_create_info(1, 1, 0);

        VkPipelineMultisampleStateCreateInfo multisample_state =
            clan::initializers::pipeline_multisample_state_create_info(VK_SAMPLE_COUNT_1_BIT);

        std::vector<VkDynamicState> dynamic_state_enables = {
            VK_DYNAMIC_STATE_VIEWPORT,
            VK_DYNAMIC_STATE_SCISSOR};
        VkPipelineDynamicStateCreateInfo dynamic_state =
            clan::initializers::pipeline_dynamic_state_create_info(dynamic_state_enables);

        VkGraphicsPipelineCreateInfo pipeline_create_info = clan::initializers::pipeline_create_info(pipeline_layout->get_handle(), render_pass);
        ...
        std::vector<VkVertexInputBindingDescription> vertex_input_bindings = {
            clan::initializers::vertex_input_binding_description(0, sizeof(ImDrawVert), VK_VERTEX_INPUT_RATE_VERTEX),
        };
        std::vector<VkVertexInputAttributeDescription> vertex_input_attributes = {
            clan::initializers::vertex_input_attribute_description(0, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(ImDrawVert, pos)),         // Location 0: Position
            clan::initializers::vertex_input_attribute_description(0, 1, VK_FORMAT_R32G32_SFLOAT, offsetof(ImDrawVert, uv)),          // Location 1: UV
            clan::initializers::vertex_input_attribute_description(0, 2, VK_FORMAT_R8G8B8A8_UNORM, offsetof(ImDrawVert, col)),        // Location 0: Color
        };
        VkPipelineVertexInputStateCreateInfo vertex_input_state_create_info = clan::initializers::pipeline_vertex_input_state_create_info();
    ...
    bool Gui::update_buffers()
    {
        ...
        if ((vertex_buffer->get_handle() == VK_NULL_HANDLE) || (vertex_buffer_size != last_vertex_buffer_size))
        {
            ...
            vertex_buffer.reset();
            vertex_buffer = std::make_unique<Buffer>(sample.get_render_context().get_device(), vertex_buffer_size,
        ...
        if ((index_buffer->get_handle() == VK_NULL_HANDLE) || (index_buffer_size != last_index_buffer_size))
        {
            ...
            index_buffer.reset();
            index_buffer = std::make_unique<Buffer>(sample.get_render_context().get_device(), index_buffer_size,
    ...
    void Gui::draw(CommandBuffer &command_buffer)
    {
        ...
        // Blend state
        clan::ColorBlendAttachmentState color_attachment{};
        ...
        clan::ColorBlendState blend_state{};
        blend_state.attachments = {color_attachment};

        command_buffer.set_color_blend_state(blend_state);

        clan::RasterizationState rasterization_state{};
        rasterization_state.cull_mode = VK_CULL_MODE_NONE;
        command_buffer.set_rasterization_state(rasterization_state);

        clan::DepthStencilState depth_state{};
        ...
        {
            std::vector<std::reference_wrapper<const clan::Buffer>> buffers;
    ...
    bool Gui::input_event(const InputEvent &input_event)
    {
        ...
        {
            bool press_down = (input_event.get_source() == EventSource::Mouse && static_cast<const MouseButtonInputEvent &>(input_event).get_action() == MouseAction::Down);
            bool press_up   = (input_event.get_source() == EventSource::Mouse && static_cast<const MouseButtonInputEvent &>(input_event).get_action() == MouseAction::Up);
