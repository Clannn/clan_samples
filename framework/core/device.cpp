...
namespace clan
{
       ...
       resource_cache{*this}
    {
        ...
        for(uint32_t queue_family_index = 0U; queue_family_index < queue_family_properties_count; ++queue_family_index){
            ...
            if(gpu.has_high_priority_graphics_queue()){
                ...
                if(graphics_queue_family == queue_family_index){
                    ...
                    for(uint32_t i = 1U; i < queue_family_property.queueCount; ++i){
        ...
        if(is_extension_supported("VK_KHR_performance_query") && is_extension_supported("VK_EXT_host_query_reset")){
    ...
    {
        return std::find_if(device_extensions.begin(), device_extensions.end(), [requested_extension](auto& device_extension){
    ...
    {
        return std::find_if(enabled_extensions.begin(), enabled_extensions.end(), [extension](auto& device_extension){
            return std::strcmp(device_extension, extension) == 0;
    ...
    const Queue& Device::get_queue_by_flags(VkQueueFlags required_queue_flags, uint32_t queue_index) const
    {
        for(uint32_t queue_family_index = 0U; queue_family_index < queues.size(); ++queue_family_index){
            const Queue& first_queue = queues[queue_family_index][0];
    ...
    {
        return to_u32(queues[queue_family_index].size());
    }
    ...
    VkBuffer Device::create_buffer(VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkDeviceSize size, VkDeviceMemory* memory, void* data)
    {
        ...
        if(data != nullptr){
            ...
            memcpy(mapped, data, to_u32(size));
    ...
    void Device::copy_buffer(Buffer& src, Buffer& dst, VkQueue queue, VkBufferCopy* copy_region)
    {
