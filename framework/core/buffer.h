...
namespace clan
{
    ...
    public:
        ...
        static std::vector<T> copy(std::unordered_map<std::string, Buffer>& buffers, const char* buffer_name)
        {
            ...
            memcpy(out.data(), buffer.get_data(), sz);
            if(!already_mapped){
                buffer.unmap();
            }
            return out;
        ...
        /**
         * @return Return the buffer's device address (note: requires that the buffer has been created with the VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT usage flag)
         */
