...
namespace clan
{
    ...
    public:
         ...
         *        primary command buffer which it inherits from must be provided
        ...
        void bind_vertex_buffers(uint32_t first_binding, const std::vector<std::reference_wrapper<const Buffer>> &buffers, const std::vector<VkDeviceSize> &offsets);
        ...
        void copy_image_to_buffer(const Image &image, VkImageLayout image_layout, const Buffer &buffer, const std::vector<VkBufferImageCopy> &regions);

        void image_memory_barrier(const ImageView &image_view, const ImageMemoryBarrier &memory_barrier) const;

        void buffer_memory_barrier(const Buffer &buffer, VkDeviceSize offset, VkDeviceSize size, const BufferMemoryBarrier &memory_barrier);
        ...
        RenderPass &get_render_pass(const RenderTarget &render_target, const std::vector<LoadStoreInfo> &load_store_infos, const std::vector<std::unique_ptr<Subpass>> &subpasses);
