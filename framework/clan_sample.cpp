...
namespace clan
{
    ...
    void ClanSample::set_render_pipeline(RenderPipeline &&rp)
    {
        render_pipeline = std::make_unique<RenderPipeline>(std::move(rp));
    }

    RenderPipeline &ClanSample::get_render_pipeline()
    {
        assert(render_pipeline && "Render pipeline was not created");
        return *render_pipeline;
    }

    bool ClanSample::prepare(Platform &platform)
    {
    ...
    void ClanSample::create_device()
    {
    }

    void ClanSample::create_instance()
    {
    }

    void ClanSample::create_render_context(Platform &platform)
    {
    ...
    void ClanSample::prepare_render_context()
    {
        render_context->prepare();
    }

    void ClanSample::update_scene(float delta_time)
    {
        if (scene)
        {
            //Update scripts
    ...
    void ClanSample::update_stats(float delta_time)
    {
        if (stats)
        {
    ...
    void ClanSample::update_gui(float delta_time)
    {
        if (gui)
        {
    ...
    void ClanSample::update(float delta_time)
    {
    ...
    void ClanSample::draw(CommandBuffer &command_buffer, RenderTarget &render_target)
    {
    ...
    void ClanSample::draw_renderpass(CommandBuffer &command_buffer, RenderTarget &render_target)
    {
    ...
    void ClanSample::render(CommandBuffer &command_buffer)
    {
        if (render_pipeline)
        {
    ...
    bool ClanSample::resize(uint32_t width, uint32_t height)
    {
    ...
    void ClanSample::input_event(const InputEvent &input_event)
    {
    ...
    Device &ClanSample::get_device()
    {
        return *device;
    }

    Configuration &ClanSample::get_configuration()
    {
        return configuration;
    }

    void ClanSample::draw_gui()
    {
    }

    void ClanSample::update_debug_window()
    {
        ...
        if (scene != nullptr)
        {
            ...
            if (auto camera = scene->get_components<sg::Camera>().at(0))
            {
    ...
    void ClanSample::set_viewport_and_scissor(CommandBuffer &command_buffer, const VkExtent2D &extent)
    {
        VkViewport viewport{};
    ...
    void ClanSample::load_scene(const std::string &path)
    {
    ...
    VkSurfaceKHR ClanSample::get_surface()
    {
        return surface;
    }

    RenderContext &ClanSample::get_render_context()
    {
        assert(render_context && "Render context is not valid");
        return *render_context;
    }

    const std::vector<const char *> ClanSample::get_validation_layers()
    {
        return {};
    }

    const std::unordered_map<const char *, bool> ClanSample::get_instance_extensions()
    {
        return instance_extensions;
    }

    const std::unordered_map<const char *, bool> ClanSample::get_device_extensions()
    {
        return device_extensions;
    }

    void ClanSample::add_device_extension(const char *extension, bool optional)
    {
        device_extensions[extension] = optional;
    }

    void ClanSample::add_instance_extension(const char *extension, bool optional)
    {
        instance_extensions[extension] = optional;
    }

    void ClanSample::set_api_version(uint32_t requested_api_version)
    {
        api_version = requested_api_version;
    }

    void ClanSample::request_gpu_features(PhysicalDevice &gpu)
    {
        // To be overridden by sample
    }

    sg::Scene &ClanSample::get_scene()
    {
        assert(scene && "Scene not loaded");
        return *scene;
    }

    bool ClanSample::has_scene()
    {
