...
KHR16BitArithmeticSample::KHR16BitArithmeticSample()
{
	...
	config.insert<clan::BoolSetting>(0, khr_16bit_arith_enabled, false);
	config.insert<clan::BoolSetting>(1, khr_16bit_arith_enabled, true);
}

bool KHR16BitArithmeticSample::prepare(clan::Platform &platform)
{
	if (!ClanSample::prepare(platform))
	{
		return false;
	}
    LOGI("prepare platform down...................");
	...
	stats->request_stats({clan::StatIndex::gpu_cycles, clan::StatIndex::frame_times});
    LOGI("request      stats     down .........................");
	gui = std::make_unique<clan::Gui>(*this, platform.get_window(), stats.get());
    LOGI("gui create down................");
	...
	blob_buffer         = std::make_unique<clan::Buffer>(device, sizeof(initial_data_fp16),
                                                      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                                      VMA_MEMORY_USAGE_GPU_ONLY);
	auto staging_buffer = std::make_unique<clan::Buffer>(device, sizeof(initial_data_fp16), VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
	...
	clan::BufferMemoryBarrier barrier;
	...
	image = std::make_unique<clan::Image>(device, VkExtent3D{Width, Height, 1},
	                                           VK_FORMAT_R16G16B16A16_SFLOAT,
	                                           VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
	                                           VMA_MEMORY_USAGE_GPU_ONLY);

	image_view = std::make_unique<clan::ImageView>(*image, VK_IMAGE_VIEW_TYPE_2D, VK_FORMAT_R16G16B16A16_SFLOAT,
	                                                    0, 0, 1, 1);
	...
	if (supported_extensions)
	{
		...
		auto &      module_fp16 =
		    device.get_resource_cache().request_shader_module(VK_SHADER_STAGE_COMPUTE_BIT,
		                                                      clan::ShaderSource{shader}, variant);
	...
	clan::ShaderSource vertex_source{"16bit_arithmetic/visualize.vert"};
	clan::ShaderSource fragment_source{"16bit_arithmetic/visualize.frag"};
...
KHR16BitArithmeticSample::VisualizationSubpass::VisualizationSubpass(clan::RenderContext &context,
                                                                     clan::ShaderSource &&vertex_source,
                                                                     clan::ShaderSource &&fragment_source) :
    clan::Subpass(context, std::move(vertex_source), std::move(fragment_source))
{
	set_output_attachments({0});
}

void KHR16BitArithmeticSample::VisualizationSubpass::draw(clan::CommandBuffer &command_buffer)
{
...
void KHR16BitArithmeticSample::VisualizationSubpass::prepare()
{
	...
	std::vector<clan::ShaderModule *> shader_modules{&vert_shader_module, &frag_shader_module};
	layout = &device.get_resource_cache().request_pipeline_layout(shader_modules);
}

void KHR16BitArithmeticSample::request_gpu_features(clan::PhysicalDevice &gpu)
{
...
void KHR16BitArithmeticSample::draw_renderpass(clan::CommandBuffer &command_buffer, clan::RenderTarget &render_target)
{
	...
	clan::ImageMemoryBarrier write_after_read_hazard;
	...
	clan::ImageMemoryBarrier to_fragment_barrier;
