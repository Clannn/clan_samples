...
#	pragma error Platform not supported
	...
	std::vector<const char *> optimal_validation_layers = clan::get_optimal_validation_layers(supported_validation_layers);
	...
	instance_info.enabledExtensionCount   = clan::to_u32(active_instance_extensions.size());
	instance_info.ppEnabledExtensionNames = active_instance_extensions.data();
	instance_info.enabledLayerCount       = clan::to_u32(requested_validation_layers.size());
                                ...
                                const std::vector<const char *> &required_device_extensions)
{
	...
	device_info.enabledExtensionCount   = clan::to_u32(required_device_extensions.size());
...
{
	clan::GLSLCompiler glsl_compiler;

	auto buffer = clan::fs::read_shader_binary(path);
...
void HelloTriangle::init_pipeline(Context &context)
{
	...
	dynamic.dynamicStateCount = clan::to_u32(dynamics.size());
	...
	pipe.stageCount          = clan::to_u32(shader_stages.size());
...
bool HelloTriangle::prepare(clan::Platform &platform)
{
	init_instance(context, {VK_KHR_SURFACE_EXTENSION_NAME}, {});

	vk_instance = std::make_unique<clan::Instance>(context.instance);
...
std::unique_ptr<clan::Application> create_hello_triangle()
{
